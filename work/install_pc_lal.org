...
1) Run (do not forget the =\=)
   ...
   #+END_SRC
2) Kill the =emacs= and run it again
   #+BEGIN_SRC sh
     $ \emacs
   #+END_SRC
   This second step will install =cask=. At the end of the process, you must get
   a message saying
   #+BEGIN_SRC sh
     cask installed: This package has been installed successfully by el-get.
   #+END_SRC
3) Kill again =emacs= and run it again. Then, inside =emacs= frame, do =ALT-x=
